...
using the 500-len pure+valid zarr (len500_stride200_fogstride100_anyfog_kaggle_defog.zarr,
filtered to purity==1.0 & validity==1.0).

Same serpentine strategy as create_fogcount_splits.py, but FOG counts
are recomputed from this dataset — patients that had negligible FOG in the
1000-len view may have more FOG here (shorter patches + any-fog labeling).

Output: configs/data/splits/kaggle_labeled/kfold_defog_fogcount_500pv_3fold{0,1,2}.yaml
"""
...
N_FOLDS = 3
MIN_FOG_PATCHES = 5   # patients below this threshold go to always-train
...
def get_patient_fog_stats():
    ...
    class_label = meta["class_label"][:]   # already binary (any-fog)
...
def write_yaml(fold: int, test: list, val: list, train: list,
    ...
    header = (
        f"# 3-fold CV (fold {fold}) stratified by fog_count — 500-len pure+valid dataset\n"
        f"# Zarr: len500_stride200_fogstride100_anyfog_kaggle_defog.zarr, filter: purity==1.0 & validity==1.0\n"
        f"# Labeling: any-fog binary (class_label). For purity==1.0 patches, any-fog == majority-vote.\n"
    ...
    out_path = out_dir / f"kfold_defog_fogcount_500pv_3fold{fold}.yaml"
    with open(out_path, "w") as f:
        ...
        for split_name, pids in [("train", sorted(train)), ("val", sorted(val)), ("test", sorted(test))]:
            ...
            for pid in sorted(str(p) for p in pids):
                f.write(f'- "{pid}"\n')
    print(f"Wrote {out_path}  (train={len(train)}, val={len(val)}, test={len(test)}, test_fog={n_test_fog}, {test_fog_pct:.1f}%)")
    return n_test_fog
...
def main():
    ...
    # Always-train: patients with negligible FOG in this dataset
    always_train = set(stats[stats["n_fog"] < MIN_FOG_PATCHES].index.tolist())
    eligible = stats[(stats["n_fog"] >= MIN_FOG_PATCHES)].copy()
    eligible = eligible.sort_values("n_fog")

    print(f"Always-train (n_fog < {MIN_FOG_PATCHES}): {len(always_train)} patients: {sorted(always_train)}")
    print(f"Test-eligible: {len(eligible)} patients")
    print(f"n_fog range (eligible): {eligible.n_fog.min()}–{eligible.n_fog.max()}")
    print(f"Total FOG patches (eligible): {eligible.n_fog.sum()}")
