#!/bin/bash
# Full finetune v3: differential LR backbone=1e-7, head=1e-3, max_epochs=100
# Motivation: fold 0 has higher FOG concentration + less notype data → backbone needs
# to move as little as possible to avoid over-sensitizing to FOG class
...
FINETUNE_CKPT_DIR="checkpoints/classification/finetune_mae_medcontext_v3"
PROBE_CKPT_DIR="checkpoints/classification/probe_mae_medcontext"

# ── Step 1: Full finetune, 3 folds ─────────────────────────────────────────
echo "=== FINETUNE v3: differential LR backbone=1e-7, head=1e-3, max_epochs=100 ==="
for fold in 0 1 2; do
  ...
  uv run python scripts/train/train_classification.py \
    ...
    "train.scheduler.T_max=100" \
    "train.trainer.max_epochs=100" \
    "train/callbacks=finetune" \
    "train.callbacks.checkpoint.dirpath=${FINETUNE_CKPT_DIR}_fold${fold}" \
    "train.callbacks.early_stopping.patience=15" \
    "data.dataloader.batch_size=256" \
    "train.logger.tags=[finetune,unfrozen,mae_2d_medcontext,exp025,fold${fold},lr_1e7]" \
    "+train.logger.name=finetune_mae_medcontext_v3_fold${fold}" \
    "+train.logger.group=finetune_mae_medcontext_exp025_v3" || echo "WARNING: fold ${fold} exited non-zero (likely WandB registry timeout)"
...
uv run python scripts/eval/eval_fogathome.py \
  --model-name finetune_mae_medcontext_exp025_v3 \
  --ckpt-pattern "${FINETUNE_CKPT_DIR}_fold{fold}/last.ckpt" \
  --n-folds 3 \
  --batch-size 64 \
  --dataset-path "len500_stride200_fogstride100_anyfog_fogathome.zarr" \
  --output-dir logs/eval/finetune_mae_medcontext_exp025_v3
